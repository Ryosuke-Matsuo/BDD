// 2020/9/13  final version for DATE2021
#include <stdio.h>
#include <stdlib.h>
#include <iostream>
#include <time.h>
...
static const int MAX_node = 100000; // the number of vertex required for constructing first BDD
...
void power_cal_red_ad( int n, double ppower, int plevel ) {
    ...
    if ( V[lowid].val == 1 ) {
        ...
        longestpath = max(longestpath, V[n].level);
    } else if ( V[lowid].val != 0 ) {
        power_cal_red_ad(lowid, V[n].power, V[n].level);
    }
    ...
    if ( V[highid].val == 1 ) {
        ...
        power_cal_red_ad(highid, V[n].power, V[n].level);
    }
}
...
void permute() {
    while ( shuffle_count < N_fact) {
        ...
        if ( perm_q < 0 ) {
            ...
            border_x = x; 
            border_y = y;
            power_cal_red_ad(initialnode.low, 10, 0); // make borderlist
            ...
            if ( reverseok == true ) {
                ...
                delay.reverseorder = longestpath;
            }
            if ( shuffle_count == 1 ) {
                firstorder.power = powersum;
                firstorder.node = nodesum;
                delay.firstorder = longestpath;
            }
...
void spec_check( double powersum, int nodesum ) {
    ...
    if ( nodesum < mnode.node ) {
        ...
        delay.mnodeminpower = longestpath;
        delay.mnodemaxpower = longestpath;
        delay.mnodeave = longestpath;
...
void one_input_result() {
    ...
    delay.average = 1;
    delay.firstorder = 1;
    delay.reverseorder = 1;
    delay.mpower = 1;
    delay.mnodeave = 1;
    delay.mnodeminpower = 1;
    delay.mnodemaxpower = 1;
}
